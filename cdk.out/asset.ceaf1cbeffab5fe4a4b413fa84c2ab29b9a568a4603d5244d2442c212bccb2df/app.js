document.getElementById("assessment-form").addEventListener("submit", async function(e) {
    ...
    try {
      ...
      sessionStorage.setItem("assessmentTopics", JSON.stringify(selectedTopics));
      sessionStorage.setItem("assessmentDifficulty", difficulty);
...
document.getElementById("start-assessment").addEventListener("click", async function() {
    try {
      ...
      // Instead of trying to fetch the assessment directly,
      // just use the information we already have from creating the assessment
      const topics = JSON.parse(sessionStorage.getItem("assessmentTopics") || "[]");
      const numQuestions = parseInt(sessionStorage.getItem("totalQuestions") || "0");
      const difficulty = sessionStorage.getItem("assessmentDifficulty") || "Medium";
      ...
      // Set initial question information
      document.getElementById("question-progress").innerText = "Question 1 of " + numQuestions;
      document.getElementById("question-topic").innerText = "Topic: " + (topics.length > 0 ? topics[0] : "General");
      document.getElementById("question-difficulty").innerText = "Difficulty: " + difficulty;
